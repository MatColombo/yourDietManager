...
function validateRecipeSource(record, path) {
  ...
  for (const key of ['cuisineIds','practicalTagIds','dietTagIds']) if (record[key] != null) for (const id of record[key]) asciiId(id, `${path}.${key}[]`);
...
export async function compileCleanCatalog(resolvedRecords, { registry = null } = {}) {
  ...
  const taxonomyTerms = [
    runtimeTerm({ termId:'flavor_neutral', taxonomyId:'flavor_profile', labelIt:'Neutro', labelEn:'Neutral' }),
    runtimeTerm({ termId:'preparation_general', taxonomyId:'preparation_technique', labelIt:'Preparazione generale', labelEn:'General preparation' })
  ];
  ...
  for (const source of [...ingredientsSource.values()].sort((a,b)=>a.id.localeCompare(b.id))) {
    ...
    const revision = {
      ...
      taxonomy:{ foodGroup:productIds.foodGroup, foodSubgroup:null, flavorProfile:source.flavorProfileId ? sourceToRuntime.get(source.flavorProfileId) : 'flavor_neutral', mealArchetypes:[...MEAL_ARCHETYPES].sort() },
