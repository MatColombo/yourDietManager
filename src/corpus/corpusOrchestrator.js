...
function allowedIngredients({ ingredientFamilies, ingredientRevisions, intent, goal }) {
  const revisionById = new Map(ingredientRevisions.map(record => [record.ingredientRevisionId, record]));
  let entries = ingredientFamilies.filter(family => family.status === 'active').map(family => ({ family, revision: revisionById.get(family.currentRevisionId) })).filter(item => item.revision);
...
function intentFeasible(intent, ingredientFamilies, ingredientRevisions) {
  if (!intent.ingredientCategories.length) return true;
  const revisionById = new Map(ingredientRevisions.map(record => [record.ingredientRevisionId, record]));
  const activeCategories = new Set(ingredientFamilies.filter(family => family.status === 'active').map(family => revisionById.get(family.currentRevisionId)?.taxonomy?.foodGroup).filter(Boolean));
...
export async function planNextBatch({ policy, snapshot, ingredientFamilies = [], ingredientRevisions = [], mode = 'build', goal = {}, seed = 'corpus-seed', targetCatalogVersion = '0.4.0-dev', referenceDataVersion = null, referenceDataDigest = null, referenceIndex = null, runId = null, createdAt = null }) {
  ...
  intents = intents.filter(intent => intentFeasible(intent, ingredientFamilies, ingredientRevisions));
  const ingredientEntries = allowedIngredients({ ingredientFamilies, ingredientRevisions, intent: intents[0] || { ingredientCategories: [] }, goal: normalizedGoal });
  ...
  const entries = allowedIngredients({ ingredientFamilies, ingredientRevisions, intent, goal: normalizedGoal });
