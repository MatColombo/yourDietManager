...
import { plannerQualityMetrics } from '../planner/qualityMetrics.js';
...
async function summarizeQuality(preview, repo) {
  ...
  return plannerQualityMetrics(preview.calendarDays || [], new Map(recipes.map(recipe => [recipe.recipeVersionId, recipe])));
}
...
export async function runPlannerValidationCase(options, { repo = repositories, registry, locale = 'it' } = {}) {
  ...
  const quality = await summarizeQuality(preview, repo);
