...
test('pre-freeze network corpus writer stays retired while the active workflow rebuilds Phase B source and Phase D validation data', async () => {
  ...
  assert.match(candidateWorkflow,/corpus:build-planner-phase-d/);
  assert.match(candidateWorkflow,/catalog:publish-planner-phase-d/);
