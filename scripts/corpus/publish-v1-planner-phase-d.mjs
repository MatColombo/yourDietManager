...
const BUNDLE='corpus/production/planner-phase-d/bundle.json';
const BUILD_EVIDENCE='corpus/production/planner-phase-d/build-evidence.json';
const OUTPUT='corpus/staging/runtime/planner-phase-d';
const EVIDENCE='corpus/production/planner-phase-d/publication-evidence.json';
const CATALOG_VERSION='1.2.0-planner-phase-d';
const BUILT_AT='2026-09-08T14:10:00.000Z';

const [bundle,buildEvidence,scanPolicy]=await Promise.all([readJson(BUNDLE),readJson(BUILD_EVIDENCE),readJson('corpus/policies/v1-default.json')]);
...
if(families.length!==1800||versions.length!==1800||ingredientFamilies.length!==600||ingredientRevisions.length!==600) throw new Error('Phase D publication requires 600/600 ingredients and 1800/1800 recipes');
for(const revision of ingredientRevisions) if(!revision.productTaxonomy?.conceptId) throw new Error(`Missing product taxonomy on ${revision.ingredientRevisionId}`);
...
if(blocking.length) throw new Error(`Phase D publication quality blockers: ${blocking.join(', ')}`);

const sourceCorpusDigest=await sha256Json({recipeDigest:buildEvidence.invariants.recipeDigestAfter,productTaxonomy:buildEvidence.productTaxonomy});
const publication={channel:'development',publicationId:`planner-phase-d-1800-${buildEvidence.invariants.recipeDigestAfter.slice(0,12)}`,sourceCorpusDigest,sourceSnapshotId:`planner-phase-d-${sourceCorpusDigest.slice(0,16)}`,requiredHumanReview:false,reviewRecipeCount:0,reviewPolicyVersion:'manual-planner-validation-phase-d',releaseEligible:false};
...
const manifest=await publishCatalogRelease({outputDir:releaseDir,catalogVersion:CATALOG_VERSION,taxonomies:bundle.taxonomies||[],taxonomyTerms:bundle.taxonomyTerms||[],referenceDataVersion:bundle.manifest.referenceDataVersion,ingredientFamilies,ingredientRevisions,recipeFamilies:families,recipeVersions:versions,builtAt:BUILT_AT,appMinVersion:'1.0.0-rc.30',locales:['it','en'],pipelineVersion:'v1-planner-phase-d-taxonomy-1',publication,packs,registry});
const evidence={schemaVersion:1,status:'planner_validation',catalogVersion:CATALOG_VERSION,appCandidateVersion:'1.0.0-rc.30',builtAt:BUILT_AT,publication,dataContract:{ingredientFamilies:600,ingredientRevisions:600,recipeFamilies:1800,recipeVersions:1800,productTaxonomyClassified:600},quantityStrategy:'fixed-template-only',recipeMutation:false,servingScalingAllowed:false,quality:{...scan.quality,exactDuplicateCount:scan.similarity.exactDuplicateCount,nearDuplicateCount:scan.similarity.nearDuplicateCount},recipeDigest:buildEvidence.invariants.recipeDigestAfter,productTaxonomy:buildEvidence.productTaxonomy,packs:Object.fromEntries(packs.map(p=>[p.packId,p.recipeVersionIds.length]))};
...
console.log(JSON.stringify({canonical,catalogVersion:manifest.catalogVersion,counts:{ingredients:ingredientFamilies.length,revisions:ingredientRevisions.length,recipes:families.length,versions:versions.length},quality:evidence.quality,productTaxonomy:evidence.productTaxonomy,evidence:EVIDENCE},null,2));
