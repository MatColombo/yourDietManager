...
const contractFile = process.argv[9] || (!/fixture|smoke|dev/i.test(targetCatalogVersion) ? 'corpus/contracts/v1-production.json' : null);
const registry = new SchemaRegistry(async file => readJson(path.join('schemas', file)));
await registry.loadAll();
const policy = await readJson(policyFile); const snapshot = await readJson(snapshotFile); const corpus = await loadCorpusInput(corpusInput); const productionContract = contractFile ? await readJson(contractFile) : null;
registry.assert('recipeCorpusPolicy', policy); registry.assert('recipeCorpusSnapshot', snapshot); if (productionContract) registry.assert('productionCorpusContract', productionContract);
let goal;
