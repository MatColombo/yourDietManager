...
function idKey(part) { return ({ ingredientFamilies:'ingredientId', ingredientRevisions:'ingredientRevisionId', recipeFamilies:'recipeId', recipeVersions:'recipeVersionId' })[part]; }
function partDir(part) { return part.startsWith('ingredient') ? 'ingredients' : 'recipes'; }
function partStem(part) { return ({ ingredientFamilies:'ingredient-families', ingredientRevisions:'ingredient-revisions', recipeFamilies:'recipe-families', recipeVersions:'recipe-versions' })[part]; }

export async function validateReleaseData({ policy, catalogVersion, ingredientFamilies, ingredientRevisions, recipeFamilies, recipeVersions, registry, requiredLocales = ['it','en'], requireCuratedIngredients = false }) {
  const issues=[]; const revisionById=new Map(ingredientRevisions.map(r=>[r.ingredientRevisionId,r])); const ingredientIds=new Set(ingredientFamilies.map(r=>r.ingredientId)); const recipeIds=new Set(recipeFamilies.map(r=>r.recipeId)); const versionIds=new Set(recipeVersions.map(r=>r.recipeVersionId));
...
export async function publishCatalogRelease({ outputDir, catalogVersion, ingredientFamilies, ingredientRevisions, recipeFamilies, recipeVersions, builtAt = new Date().toISOString(), appMinVersion = '0.4.0', locales = ['it','en'], pipelineVersion = 'recipe-pipeline-1', shardSize = 250, packs = null, registry = null }) {
  await rm(outputDir,{recursive:true,force:true}); const dataDir=path.join(outputDir,'data'); await mkdir(dataDir,{recursive:true});
  const parts={ ingredientFamilies, ingredientRevisions, recipeFamilies, recipeVersions }; const manifestParts={};
  ...
  const allVersionIds=recipeVersions.map(record=>record.recipeVersionId); const derivedPacks=packs || [
    ...
    {packId:'vegetarian',labelKey:'catalog.pack.vegetarian.label',descriptionKey:'catalog.pack.vegetarian.description',required:false,estimatedBytes:0,recipeVersionIds:recipeVersions.filter(v=>(v.tags.diet||[]).includes('vegetarian')).map(v=>v.recipeVersionId)}
  ];
  const manifest={schemaVersion:1,catalogVersion,builtAt,appCompatibility:{minVersion:appMinVersion,maxVersion:null},locales,pipelineVersion,calculationAlgorithmVersion:CALCULATION_ALGORITHM_VERSION,...manifestParts,packs:derivedPacks}; registry?.assert('catalogManifest',manifest); await writeFile(path.join(dataDir,'catalog-manifest.json'),jsonText(manifest)); return manifest;
}
