...
import { APP_VERSION as BASELINE_APP_VERSION, BACKUP_FORMAT_VERSION as BASELINE_BACKUP_FORMAT_VERSION, CONTENT_SCHEMA_VERSION as BASELINE_CONTENT_SCHEMA_VERSION, DB_VERSION as BASELINE_DB_VERSION, PRE_V1_DATA_EPOCH as BASELINE_PRE_V1_DATA_EPOCH } from '../../specs/revision_v2/baseline/runtime/src/db/constants.js';
...
const [pkg,baselinePkg,catalog,freeze,acceptance,handoff,phaseD,phaseF,promotionScript,workflow] = await Promise.all([
...
const runtime={appVersion:BASELINE_APP_VERSION,dbVersion:BASELINE_DB_VERSION,contentSchemaVersion:BASELINE_CONTENT_SCHEMA_VERSION,backupFormatVersion:BASELINE_BACKUP_FORMAT_VERSION,preV1DataEpoch:BASELINE_PRE_V1_DATA_EPOCH};
const gate=await evaluateV1ReleaseState({pkg:baselinePkg,catalog,freeze,acceptance,phaseD,phaseF,runtime});
...
const isCandidate=BASELINE_APP_VERSION===freeze.appCandidateVersion;
const isStable=BASELINE_APP_VERSION===STABLE_VERSION;

check('phase-h-version-boundary',pkg.version===APP_VERSION&&/^1\./.test(APP_VERSION)&&baselinePkg.version===BASELINE_APP_VERSION&&(isCandidate||isStable),`package=${pkg.version}, runtime=${APP_VERSION}, historical=${BASELINE_APP_VERSION}, frozenCandidate=${freeze.appCandidateVersion}`);
...
check('phase-h-frozen-runtime',BASELINE_DB_VERSION===6&&BASELINE_CONTENT_SCHEMA_VERSION===3&&BASELINE_BACKUP_FORMAT_VERSION===1&&BASELINE_PRE_V1_DATA_EPOCH==='v1-planner-phase-d-epoch-1'&&DB_VERSION>=BASELINE_DB_VERSION&&CONTENT_SCHEMA_VERSION>=BASELINE_CONTENT_SCHEMA_VERSION&&BACKUP_FORMAT_VERSION>=BASELINE_BACKUP_FORMAT_VERSION&&PRE_V1_DATA_EPOCH===BASELINE_PRE_V1_DATA_EPOCH,`current db=${DB_VERSION}, content=${CONTENT_SCHEMA_VERSION}, backup=${BACKUP_FORMAT_VERSION}; frozen db=${BASELINE_DB_VERSION}, content=${BASELINE_CONTENT_SCHEMA_VERSION}, backup=${BASELINE_BACKUP_FORMAT_VERSION}, epoch=${BASELINE_PRE_V1_DATA_EPOCH}`);
