...
check('phase-c-app-version', pkg.version === APP_VERSION && /^1\.(?:0\.0(?:-rc\.\d+)?|1\.0-dev\.r\d+)$/.test(APP_VERSION), `package=${pkg.version}, runtime=${APP_VERSION}`);
...
check('replacement-full-retrieval', /MAX_PLANNER_CANDIDATES_PER_ARCHETYPE/.test(effectivePlanService) && /candidateService\.retrieve\(context\.mealClass\.mealArchetype, \{[\s\S]*?limit: MAX_PLANNER_CANDIDATES_PER_ARCHETYPE,[\s\S]*?eligibilityContexts:/.test(effectivePlanService) && !/excludeAllergens\s*:/.test(effectivePlanService), 'replacement uses the shared 500-candidate bound and delegates canonical hard eligibility to PlanCandidateService');
check('allergen-visible-to-hard-filter', /query\.retrieve\(archetype, \{[\s\S]*?limit,[\s\S]*?eligibilityContexts/.test(generationService) && /hardFilterRecipe\(version,/.test(candidateService) && /hardRejectionCounts/.test(candidateService) && !/excludeAllergens\s*:/.test(generationService), 'candidate retrieval applies the canonical hard filter with eligibility contexts and preserves rejection diagnostics');
...
check('pwa-phase-c-shell', shellCacheVersion >= 37 && workerDataCacheVersion >= 17 && offlineDataCacheVersion === workerDataCacheVersion && ['src/services/plannerValidationService.js','src/planner/validationProfiles.js','src/ui/plannerValidationPage.js'].every(file => worker.includes(file)), `shell=v${shellCacheVersion}, data=v${workerDataCacheVersion}, Phase C modules precached`);
