...
check('phase-e-version',/^1\./.test(pkg.version)&&constants.includes(`APP_VERSION = '${pkg.version}'`),`package=${pkg.version}`);
...
const shellVersion=Number(sw.match(/ydm-shell-v(\d+)-/)?.[1]||0);
check('pwa-shell-phase-e',shellVersion>=37&&sw.includes('src/services/manualAcceptanceService.js')&&sw.includes('src/ui/manualAcceptancePage.js'),`shell v${shellVersion} precaches Phase E harness`);
