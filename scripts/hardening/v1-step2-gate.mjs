...
check('app-version-sync', pkg.version === APP_VERSION && /^1\.\d+\.\d+(?:-[0-9A-Za-z.-]+)?$/.test(pkg.version), `package=${pkg.version}, runtime=${APP_VERSION}`);
...
check('plan-recipe-metrics-scope', /function recipePills\(state, recipe\)/.test(planUi) && !/(?<!function )recipePills\((?!state,)/.test(planUi), 'translation state is explicitly passed to every recipe metrics rendering');
...
check('shopping-recalculation-key', shoppingUi.includes("await state.repo.getMeta('planUpdatedAt')") && shoppingUi.includes('const calculationKey = shoppingCalculationKey(state, ui, planUpdatedAt)') && shoppingUi.includes('ui.calculated = await calculateShoppingList(') && shoppingUi.includes('ui.calculationKey = calculationKey'), 'shopping view keys the derived projection to planUpdatedAt and recomputes the authoritative list');
...
const offlineDataVersion = Number(offline.match(/ydm-data-v(\d+)-/)?.[1]);
const workerDataVersion = Number(worker.match(/ydm-data-v(\d+)-/)?.[1]);
check('offline-cache-parity', Number.isInteger(offlineDataVersion) && offlineDataVersion >= 17 && offlineDataVersion === workerDataVersion, `direct offline cache and service worker share data cache v${workerDataVersion || '?'}`);
const workerShellVersion = Number(worker.match(/ydm-shell-v(\d+)-/)?.[1]);
check('shell-cache-bumped', Number.isInteger(workerShellVersion) && workerShellVersion >= 37, `planner-validation shell assets use shell cache v${workerShellVersion || '?'} (>=37)`);
